...
// Chgv implements values that track changes with a simple flag-set of
...
type Chgv[T comparable] struct{ v T }

// Get returns the current value.
func (cv Chgv[T]) Get() T {
	return cv.v
}
...
func (cv *Chgv[T]) Set(v T, chg Flags) Flags {
	if cv.v == v {
		return 0
	}
	cv.v = v
